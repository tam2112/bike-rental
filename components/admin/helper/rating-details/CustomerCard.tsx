...
export default function CustomerCard({ rating }: { rating: RatingType }) {
    return (
        <div className="bg-white dark:bg-slate-850 rounded-xl p-6 shadow-sm border border-slate-200 dark:border-slate-700 flex flex-col gap-4">
            <div className="flex items-center justify-between">
                ...
                <Link
                    href={`${customerPath}/chi-tiet/${rating.user.id}`}
            ...
            <div className="flex items-center gap-4 py-2">
                ...
                <div>
                    <p className="font-bold text-slate-900 dark:text-white text-base">{rating.user.fullName}</p>
                    <p className="text-slate-500 dark:text-slate-400 text-sm capitalize">{rating.user.role.name}</p>
                </div>
            </div>
            <div className="space-y-3 pt-2 border-t border-slate-100 dark:border-slate-700">
                <div className="flex justify-between items-center">
                    ...
                    <Link
                        ...
                        href="tel:0909123456"
                    >
                        {rating.user.phone ? formatPhoneNumberRegex(rating.user.phone) : '-------'}
                    </Link>
                </div>
                <div className="flex justify-between items-center">
                ...
                <div className="flex justify-between items-center">
                    ...
                    <span className="text-slate-900 dark:text-slate-200 font-medium text-sm">
                        {rating.user.idCard ? formatIDCard(rating.user.idCard) : '-------'}
                    </span>
                </div>
            </div>
        </div>
