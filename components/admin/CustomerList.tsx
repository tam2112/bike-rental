...
import { Filters, MobileList, Table } from './helper/cus-list';
import { LoadingSkeleton } from './helper/skeleton';

export default function CustomerList() {
    const { state, actions } = useCustomerLogic();

    return (
        <div className="flex-1 flex flex-col h-full bg-background-light dark:bg-background-dark overflow-hidden">
            <PageHeader breadcrumbs={[{ href: adminPath, label: 'Tổng quan' }]} current="Quản lý khách hàng" />
                    ...
                    <div className="space-y-4">
                        <AnimatePresence mode="wait">
                            {state.isPending ? (
                                ...
                                >
                                    {/* Mobile View */}
                                    <MobileList users={state.paginatedUsers} />
                                    {/* Desktop View */}
                                    <Table users={state.paginatedUsers} />
                                </motion.div>
                            )}
                        </AnimatePresence>
                    </div>
